...
- **Pas d'injection de commande possible** : `subprocess.Popen(cmd_args, ...)` reçoit une **liste** d'arguments, jamais `shell=True`, et rien dans `cmd_args` n'est dérivé du contenu de l'image ou d'une entrée utilisateur — `lang` est la constante fixe `"fra"` (`OCR_LANGUAGE`), le seul paramètre `config` passé par notre code est `--psm N`, où `N` est un entier (`OCR_PSM`, variable d'environnement lue au démarrage, jamais une entrée utilisateur) validé contre la liste fermée `{3, 4, 6, 11}` — toute autre valeur fait échouer le démarrage de l'application (`RuntimeError`), avant tout traitement ; s'y ajoute l'argument fixe `-c tessedit_create_tsv=1` côté pytesseract.
